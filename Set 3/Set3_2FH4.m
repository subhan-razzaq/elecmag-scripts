% Set 3 Exercise MATLAB Code
% Subhan Razzaq - 400557958
%
% Electric field at the origin from a mix of point charges and a line charge.
% Two 8 nC point charges sit at (0,1,1) and (0,-1,1), and a uniform line
% charge of 4 nC/m runs along a diagonal segment in the xy plane centered at
% (3.5,3.5,0). The point charge contributions come straight from Coulomb's
% law. The line charge is broken into 100000 small segments, and the field
% from each one gets added up so the sum approximates the integral. The three
% contributions are combined and the total field vector is printed.
...
Epsilono=8.85e-12;%Permitivity of free space
...
length=7*sqrt(2);%the length of the line
dL_V = (length/Number_of_L_Steps) * (1/sqrt(2)) * [-1 1 0];
...
E = E1 + E2 + EL;
fprintf("The electric field at point P(0,0,0) is [%.3f, %.3f, %.3f] N/m\n", E(1), E(2), E(3));